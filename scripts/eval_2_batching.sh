#!/bin/bash
# ================================================================
# EVAL 2: Batch Size Evaluation (MongoDB workload a)
# Tests various BATCHSIZE values: 1, 10, 50, 100, 500, 1000, 2000
# INDEP_RATIO fixed at 100.0.
...
RESULT_ROOT="${SCRIPT_DIR}/results/eval2_batching"
...
# Test cases: BATCHSIZE values
TEST_CASES=(1 10 50 100 500 1000 2000)

echo "=============================================="
echo "EVAL 2: Batch Size (MongoDB workload a)"
echo "=============================================="
...
start_workload_nodes() {
    ...
    for i in "${!SERVER_IPS[@]}"; do
        ip="${SERVER_IPS[$i]}"
        remote_exec "$ip" "pkill -f 'woc.*-path' 2>/dev/null || true; nohup '$REMOTE_DIR/$BINARY' -id=$i -path='$CONFIG_PATH' -et=1 -n=$NUM_SERVERS -t=$THRESHOLD -b=$batchsize -mode=1 -mcli=$MONGO_CLIENT_POOL -mload='$WORKLOAD' -bcomp=object-specific -indep=$INDEP_RATIO -pipeline=$PIPELINE_MODE -maxinflight=$MAX_INFLIGHT -log=$LOG_LEVEL -ep=true -role=0 > '$LOG_DIR/server_${i}_batch_${batchsize}.log' 2>&1 &"
    done
    ...
    for i in "${!CLIENT_HOST_IPS[@]}"; do
        ...
        remote_exec "$ip" "pkill -f 'woc.*-path' 2>/dev/null || true; nohup '$REMOTE_DIR/$BINARY' -id=$client_id -path='$CONFIG_PATH' -et=1 -n=$NUM_SERVERS -t=$THRESHOLD -b=$batchsize -mode=1 -mload='$WORKLOAD' -bcomp=object-specific -indep=$INDEP_RATIO -pipeline=$PIPELINE_MODE -maxinflight=$MAX_INFLIGHT -log=$LOG_LEVEL -ops=0 -pinserver=$pin_server -role=1 > '$LOG_DIR/client_${i}_batch_${batchsize}.log' 2>&1 &"
    done
}
...
start_cluster() {
    local batchsize=$1
    local test_num=$2
    local label="batch_${batchsize}"

    echo ""
    echo "--- Test $test_num: BATCHSIZE=$batchsize ---"

    start_workload_nodes "$batchsize"
...
for batchsize in "${TEST_CASES[@]}"; do
    start_cluster "$batchsize" "$test_num"
    test_num=$((test_num + 1))
done
