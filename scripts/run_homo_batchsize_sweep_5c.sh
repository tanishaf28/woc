#!/bin/bash
# ================================================================
# HOMOGENEOUS EVAL 6: Batch Size Sweep x Cluster Size, 5-client pool
#
# Homogeneous-cluster counterpart of eval_6_batchsize_size_sweep.sh (itself
# mirrored from EPaxos's run_hetero_batchsize_sweep_10c.sh). Sweeps
# BATCHSIZE over 1,10,50,100,500,1000,2000 (indep=90, msgsize=512) across
# cluster sizes n=3,5,7,11, against a fixed pool of 5 clients -- all
# servers/clients are sliced from the single flat config/cluster_homo.conf
# pool (server id i always maps to row i regardless of cluster size, so no
# per-size config files are needed, unlike the hetero sweep).
#
...
RESULT_ROOT="${SCRIPT_DIR}/results/homo_batchsize_sweep_5c"
...
if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash run_homo_batchsize_sweep_5c.sh

Sweeps BATCHSIZE over 1,10,50,100,500,1000,2000 (matches
eval_6_batchsize_size_sweep.sh / run_hetero_batchsize_sweep_10c.sh) with
INDEP_RATIO=90.0, MSG_SIZE=512 fixed, across cluster sizes n=3,5,7,11
(homogeneous), against a fixed pool of 5 clients. All servers/clients are
sliced from the single flat config/cluster_homo.conf pool.
...
Results archived under: results/homo_batchsize_sweep_5c/<timestamp>/n<size>/<label>/
EOF
    exit 0
fi
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║   BATCHSIZE SWEEP x CLUSTER SIZE, 5-CLIENT POOL (WOC HOMO)     ║"
...
echo ""
echo "=================================================="
echo " Batchsize sweep (5-client pool, homogeneous) complete"
echo "=================================================="
