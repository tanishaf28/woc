#!/bin/bash
# ================================================================
# WOC MongoDB Ratio Sweep, HETEROGENEOUS 5-NODE cluster.
#
# Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (mirrors this repo's
# eval_5_i2d_size_sweep.sh / epaxos port's run_mongodb_ratio_sweep_5n.sh)
# with BATCHSIZE=1 fixed, against the MongoDB-backed cluster
...
against the heterogeneous 5-node WOC MongoDB replica set
(start_mongodb_hetero.sh, -et=1).
...
run_case() {
    ...
    CLUSTER_ACTIVE=true
    INDEP_RATIO="$indep" BATCHSIZE=1 NUM_OBJECTS=1000 READ_RATIO=0.0 \
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║      WOC MONGODB RATIO SWEEP, HETEROGENEOUS 5-NODE             ║"
echo "╚════════════════════════════════════════════════════════════════╝"
...
for indep in "${TEST_CASES[@]}"; do
    run_case "n5_woc_mongo_ratio_${indep}" "$indep"
done
...
echo ""
echo "=================================================="
echo " WOC MongoDB ratio sweep complete"
echo "=================================================="
