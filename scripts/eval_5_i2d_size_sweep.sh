#!/bin/bash
# ================================================================
# EVAL 5: Indep-to-Dependent Ratio Sweep x Cluster Size (PlainMsg)
#
# Sweeps INDEP_RATIO (100,90,80,60,40,20,10,0 -- same points as
# eval_1_indep_ratio.sh / run_hetero5_netem_i2d_sweep.sh) for each
# cluster size in CLUSTER_SIZES (3,5,7,11 servers), using the fixed
# 11-IP server pool already used by config/cluster_hetero_{3,5,7,11}n_*.conf,
# paired with a dedicated 10-IP client pool (config/cluster_hetero_{n}n_10c.conf).
# Not MongoDB-backed (-et=0), same as eval_4_read_ratio.sh.
#
# Fixed across the sweep: BATCHSIZE=1, MSG_SIZE=512.
#
# Delegates cluster lifecycle to start_cluster_hetero.sh/stop_cluster_hetero.sh
# via env-var injection instead of driving SSH/scp/nohup inline here --
# mirrors EPaxos's run_hetero_ratio_sweep_10c.sh port of this same script, so
# a fix to the shared start/stop scripts (e.g. stop_cluster_hetero.sh's
# SERVER_COUNT/CLIENT_COUNT slicing fix) benefits this sweep automatically
# instead of needing to be copied into a second inline driver.
# ================================================================
...
RESULT_ROOT="${SCRIPT_DIR}/results/eval5_i2d_size_sweep"
...
# since, unlike EPaxos's port, this repo's start_cluster_hetero.sh doesn't
# cycle/reuse client VMs beyond the pool it's given).
CLIENT_COUNT="${CLIENT_COUNT:-10}"
TEST_CASES=(100.0 90.0 80.0 60.0 40.0 20.0 10.0 0.0)

if [[ "${1:-}" == "--help" ]]; then
    cat <<'EOF'
Usage: bash eval_5_i2d_size_sweep.sh

Sweeps INDEP_RATIO over 100,90,80,60,40,20,10,0 (matches
run_hetero_ratio_sweep_10c.sh's EPaxos port of this script) with
...
Results archived under: results/eval5_i2d_size_sweep/<timestamp>/n<size>/<label>/
EOF
    exit 0
fi
...
cleanup_all_nodes() {
    ...
    echo "=================================================="
    local all_ips=()
    printf '%s\n' "${ALL_SERVER_IPS[@]}" "${ALL_CLIENT_IPS[@]}" | sort -u > /tmp/eval5_ip_pool.$$
    mapfile -t all_ips < /tmp/eval5_ip_pool.$$
    rm -f /tmp/eval5_ip_pool.$$
...
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║     RATIO SWEEP x CLUSTER SIZE, 10-CLIENT POOL (WOC)           ║"
