...
# Same 11-server / 10-client pools as start_cluster_hetero.sh, sliced below
# by SERVER_COUNT/CLIENT_COUNT - keep in sync with that script's
# ALL_SERVER_IPS/ALL_CLIENT_IPS. Previously this array only had the first 5
# server IPs and 2 client IPs, so stopping a 7- or 11-server hetero cluster
# (or one started with more than 2 clients) silently left every process
# beyond that hardcoded prefix running, since the SERVER_COUNT bounds check
# rejected anything above 5 and the STOP CLIENTS loop only ever iterated
# over the 2 hardcoded client IPs regardless of CLIENT_COUNT.
ALL_SERVER_IPS=(
...
if [ "$CLIENT_COUNT" -lt 0 ] || [ "$CLIENT_COUNT" -gt "${#ALL_CLIENT_IPS[@]}" ]; then
    echo "ERROR: CLIENT_COUNT must be between 0 and ${#ALL_CLIENT_IPS[@]} (got $CLIENT_COUNT)"
    exit 1
fi
