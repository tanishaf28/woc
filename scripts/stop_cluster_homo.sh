#!/bin/bash
# ================================================================
# WOC Cloud Cluster Stopper - HOMOGENEOUS CLUSTER
# ================================================================
...
# Same flat cluster_homo.conf pool as start_cluster_homo.sh -- server id i
# always maps to row i regardless of cluster size, so SERVER_COUNT/CLIENT_COUNT
# just slice a prefix/next-slice of it (keep in sync with that script).
...
REQUIRED_IPS=$((SERVER_COUNT + CLIENT_COUNT))
if [ "$SERVER_COUNT" -lt 1 ] || [ "$CLIENT_COUNT" -lt 0 ] || [ "$REQUIRED_IPS" -gt "${#ALL_IPS[@]}" ]; then
    echo "ERROR: ${CONFIG_LOCAL} has ${#ALL_IPS[@]} IPs, need ${REQUIRED_IPS} (SERVER_COUNT=${SERVER_COUNT} + CLIENT_COUNT=${CLIENT_COUNT}, both >= 0, SERVER_COUNT >= 1)"
    exit 1
fi
...
echo "=================================================="
echo " WOC HOMOGENEOUS Cluster Shutdown"
