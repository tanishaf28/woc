...
type ObjectState struct {
	...
	WeightCache       []float64       // Pre-computed array for lock-free access
...
func (o *ObjectState) GenerateWeights(numReplicas int) {
	...
	o.WeightCache = make([]float64, numReplicas) // Pre-allocate cache for lock-free access
	o.TotalWeight = 0

	for id := 0; id < numReplicas; id++ {
		w := math.Pow(o.RValue, float64(numReplicas-1-id))
		o.Weights[id] = w
		o.WeightCache[id] = w // Cache weight for lock-free access
		o.TotalWeight += w
		if w > o.MaxWeight {
			o.MaxWeight = w
		}
